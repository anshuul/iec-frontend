...
const POListItemTable = ({ productionStep }) => {
  ...
  const handleDeleteClick = async (poNo, POListNo) => {
    try {
      setLoading(true);
      await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/customerPO/deleteListItem?poNo=${poNo}&listItemNo=${POListNo}`
      );
      // Filter out the deleted row from rowData
      setRowData((prevRowData) =>
        prevRowData.filter((row) => row.POListNo !== POListNo)
      );
    } catch (error) {
  ...
  const CustomButtonComponent = (props) => {
    ...
    return (
      <div className="flex flex-row items-center gap-2 pt-1 ag-theme-alpine">
        ...
        {/* Delete Button */}
        <button
          onClick={() => handleDeleteClick(data.poNo, data.POListNo)}
