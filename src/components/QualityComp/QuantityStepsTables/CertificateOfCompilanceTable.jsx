...
const CertificateOfCompilanceTable = ({ qualityStep }) => {
      ...
      try {
        ...
        if (selectedPOListItem) {
          ...
          response = await axios.get(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/quality/coc/getBy-listItemNo?poNo=${parsedPOListItem.poNo}&listItemNo=${parsedPOListItem.POListNo}`
          );
        } else {
          response = await axios.get(
  ...
  const handleClick = () => {
    router.push(`/quality/${qualityStep}/certificateOfCompilanceForm`);
  };
...
export default CertificateOfCompilanceTable;
