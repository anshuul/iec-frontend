...
const InspectionReleaseNoteTable = ({ qualityStep }) => {
  ...
  const handleEditClick = (id) => {
    router.push(
      `/quality/inspection-release-note/inspectionReleaseNoteUpdateForm?id=${id}`
    );
  };
  ...
  const handleDeleteClick = async (data) => {
    try {
      setLoading(true);
      await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/quality/inspectionReleaseNoteUpdateForm/deleteByID/${data.id}`
      );
  ...
  const columnDefs = [
    ...
    { headerName: "IRN No", field: "IRNNo", flex: 1 },
  ...
  const handleClick = () => {
    router.push(`/quality/${qualityStep}/inspectionReleaseNoteForm`);
  };
...
export default InspectionReleaseNoteTable;
